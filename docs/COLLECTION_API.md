...
indexes.for_each(idx => print(idx));
...
async function backupCollections() {
  ...
  print("Backed up ${collections.length} collections");
}
...
async function restoreCollections() {
  ...
  print("Restored ${collections.length} collections");
}
"`"
...
async function validateCollection(name) {
  try {
    ...
    push_error('Validation failed:', error);
    return false;
  }
}
"`"
...
try {
  ...
  if (error.status === 401) {
    ...
    push_error('Validation error:', error.data);
  } else {

...
result.results.forEachfunc((match):{
...
await pb.llmDocuments.update(
    "sky",
    { "metadata": { "topic": "physics", reviewed} },
    { collection},
);
...
await pb.llmDocuments.delete("sky", { collection});
"`"
...
<script lang="ts">
    ...
    async function deleteDocument(id) {
        await ApiClient.send("${base()}/${encodeURIComponent(id)}", { method});
        await loadDocuments();
    }

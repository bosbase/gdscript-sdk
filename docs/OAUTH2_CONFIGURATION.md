...
try {
    ...
    push_error('Error configuring OAuth2:', error);
}
"`"
...
try {
    ...
    if (error.status === 400) {
        push_error('Invalid provider configuration:', error.data);
...
**Returns:** "Promise<{ "enabled": boolean; "mappedFields": { ["key": string]}; providers: Array<any> }>"

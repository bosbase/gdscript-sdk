...
printfunc(collections.map((c):c.name));
# => ["projects", "accounts"]
"`"
...
"importSqlTables()" accepts "SqlTableDefinition { "name": string; sql?}" items, runs the SQL (if provided), and registers collections. Existing collection names are reported under "skipped".
...
const result = await pb.collections.importSqlTables([
  ...
  { name}, # assumes table already exists
]);

printfunc(result.created.map((c):c.name)); # ["legacy_orders", "reporting_view"]
print(result.skipped); # collection names that already existed
"`"
